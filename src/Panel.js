...
function Panel() {
  ...
  const fetchData = async () => {
    const response = await fetchWithToken('http://idoktor.org:8080/api/Doctors/GetUnverifiedDoctors', {
    method: 'GET'
  });
  ...
  const acceptDoctor = async (id) => {
    const response = await fetchWithToken(`http://idoktor.org:8080/api/Doctors/VerifyDoctor/${id}`, {
      method: 'PUT'
    });
  ...
  const declineDoctor = async (id) => {
    const response = await fetchWithToken(`http://idoktor.org:8080/api/Doctors/RejectDoctor/${id}`, {
      method: 'PUT'
    });
                  ...
                  <tbody>
                    {doctors.map(doctor => {
                      ...
                      <td className="d-none d-xl-table-cell">
                          <a href={`http://idoktor.org:8080/${doctor.verificationDocumentPath}`} target="_blank" rel="noopener noreferrer">Verifikasiya sənədi</a>
                      </td>
                      <td>
